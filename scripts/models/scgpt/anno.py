...
parser.add_argument('-t','--test', type=str, help='test dataset name')
...
parser.add_argument('-k','--kfold', type=int, default=-1, help='')            # 我们强制更改了scGPT本身就有的拆分20% 默认请填0, -1代表train 是全部
parser.add_argument('-b','--batchsize', type=int, default=96, help='')          # 根据zero测试一下
parser.add_argument('-m','--mouse', action="store_true", help='Mouse Data')     # 一般都是人，看一下物种吧
parser.add_argument('-z','--zero', action="store_true", help='Include zero gene')   # 默认不包括，别传
parser.add_argument('-c','--continuous', action="store_true", help='is continuous but not binned?') # 默认是false 不要填, 开启bin
parser.add_argument('-x','--integ', action="store_true", help='use integ dataset')  # 从哪读取数据， 默认都是从anno读，不用输入
parser.add_argument('-n','--nhvg', type=int, default=3000, help='n HVG in anndata')  # hvg数量
args = parser.parse_args()  # 获取所有参数
# args = parser.parse_args(parse_cmd)
SUFFIX_STORE = "../data/integ/" if args.integ else "../data/"

args_bin = None if args.continuous else 51  # 默认是flase
...
# [ Argparser Settings >>
custom_name = args.dataName #"hLUSC" # "hBreast"
load_model = args.model_dir
dataset_name = custom_name  #"hLUSC"
load_data_train = args.train
load_data_test = args.test
ori_celltype_col = args.annoKey   #"cell_type_major" #author_cell_type
kfold = args.kfold
batch_size = args.batchsize     # 16 can run | default 32
# ori_batch_col = "sample_uuid" annotation不需要batch [from tutorial]
...
print(f"Bin: {args_bin}, CONTINUOUS:{args_continuous}, kfold:{kfold}")
print(f"mg: {mg_upper}")
print(f"--------------------\n Train from: {load_data_train} \n Test from: {load_data_test}")




n_hvg = args.nhvg or 3000 # 3000
print(f"Use hvg counts: {n_hvg}")
...
hyperparameter_defaults = dict(
    seed=0,
    dataset_name=dataset_name,  # <Custom-Settings>
    do_train=True,
    load_model=load_model,   # <Custom-Settings>
    mask_ratio=0.0,
    epochs=10,
    n_bins=args_bin,
...
max_seq_len = n_hvg + 1  # <Custom-Settings>
n_bins = config.n_bins

# input/output representation
input_style = "normed_raw" if args_continuous else "binned"  # "normed_raw", "log1p", or "binned"
output_style = "normed_raw" if args_continuous else "binned"  # "normed_raw", "log1p", or "binned"
...
# +---------------------------+
# 读取并分组 train 与 valid
adata = sc.read(load_data_train)
del adata.layers    # del
adata_test = sc.read(load_data_test)
del adata_test.layers    # del

# logging
logger.info(f"trainSet_Shape \t [{adata.shape}]")
logger.info(f"testSet_Shape \t [{adata_test.shape}]")
logger.info("Param Info------------------------------------------------")
logger.info(f"celltype: {custom_name} -> {ori_celltype_col}")
logger.info(f"hvg counts: {n_hvg}")
logger.info(f"kfold: {kfold}")
logger.info(f"batch size: {batch_size}")
logger.info(f"mg: {mg_upper}")
logger.info("----------------------------------------------------------")
# 预处理label
adata.obs["celltype"] = adata.obs[ori_celltype_col].astype("category")
adata_test.obs["celltype"] = adata_test.obs[ori_celltype_col].astype("category")
...
if mg_upper:
    ...
    temp = list(adata_test.var.index)
    gene_list=[item.upper() for item in temp]
    adata_test.var.index = gene_list

# Gene Symbol Key
if gene_symbols_key is not None:
    adata.var = adata.var.set_index(gene_symbols_key, inplace=True)
...
with open(save_dir / "id_map.pkl", "wb") as f:
    pickle.dump(id2type_save_dict, f)
...
logger.info("Preprocessor Settings: raw:{data_is_raw}, norm:{normalize_total}")
...
print(f"use Bin Layer:{adata.layers['X_normed'].max()}")
# 预处理后 设置模型输入类型及 对应key入口
...
# 替换原来的 train_test_split 函数进行valid kfold拆分
def my_train_test_split(counts, labels, batchs, k):
    mask = trainSplit(counts.shape[0],0.2, max(k,0))
    t_counts = counts[mask] if k > 0 else counts
    v_counts = counts[~mask]
    t_labels = labels[mask] if k > 0 else labels
    v_labels = labels[~mask]
    t_batchs = batchs[mask] if k > 0 else batchs
    v_batchs = batchs[~mask]
    return t_counts, v_counts, t_labels, v_labels, t_batchs, v_batchs
...
(
    ...
    train_batch_labels,
    valid_batch_labels,
) = my_train_test_split(all_counts, celltypes_labels, batch_ids, k=kfold)
...
def evaluate(model: nn.Module, loader: DataLoader, return_raw: bool = False, train_flag=True) -> float:
    """
    ...
    with torch.no_grad():
        for batch_data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=config.amp):
                ...
                if not train_flag:
                    if cls_emb is None:
                        cls_emb = output_dict["cell_emb"].detach().cpu().numpy()
                    else:
                        cls_emb = np.concatenate((cls_emb, output_dict["cell_emb"].detach().cpu().numpy()), axis=0)
                    # 组合为numpy数组
                # 判断test才添加
    ...
    if return_raw:
        return np.concatenate(predictions, axis=0), cls_emb
    #
...
def test(model: nn.Module, adata: DataLoader) -> float:
    ...
    return predictions, celltypes_labels, results, cls_emb


# test Done
preprocessor(adata_test, batch_key=None)    # move
predictions, labels, results, cls_emb = test(best_model, adata_test)    # model_dicts
...
emb = ad.AnnData(X=cls_emb, obs=adata_test_raw.obs)
emb.write(save_dir / "test.anno.h5ad")


print("Writed Embeddings, Over Task...")
anno = pd.DataFrame([list(adata_test.obs[ori_celltype_col]), list(adata_test.obs['predictions'])], 
                    index=['True', 'Pred'], columns=adata_test.obs_names).T
anno.to_csv(f"{save_dir}/anno.tsv", sep='\t')
